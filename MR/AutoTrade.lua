-- Bun222222222222222dled Universal Auto-Trade. Generated by tools/bundle.py. Do not edit.
...
-- Resolve the trade remotes. The base resolves them under
-- workspace.__THINGS.__REMOTES (confirmed from the deob), with a
-- LocalPlayer fallback and a descendant search as last resort.
function M.resolveRemotes(timeout)
  ...
  local function tryParent(parent)
    ...
    out.__container = remotes
    return out
  end

  -- Primary: workspace (the base's literal path). Wait for it to exist first.
  ...
  -- Last resort: find any __REMOTES anywhere.
  local remotes
  pcall(function()
  ...
  if remotes then
    ...
    for _, n in ipairs(names) do
      ...
      if not r then return nil, "missing remote " .. n .. " (via search)" end
      out2[n] = r
    end
    out2.__container = remotes
    return out2, "search:" .. remotes:GetFullName()
  end

  return nil, "no __THINGS.__REMOTES found (workspace/localplayer/search)"
end

-- Load the untouched obfuscated base and return whatever it returns.
function M.loadBase(baseUrl)
  local ok, res = pcall(function() return loadstring(game:HttpGet(baseUrl))() end)
  return ok, res
end

-- Force the base's own SmartTrade OFF via its saved-settings file.
...
-- Acquire the client trade-data root, exactly as the base does:
--   require(ReplicatedStorage:WaitForChild("Framework"):WaitForChild("Library"))
-- The returned module exposes .incomingInvites, .activeTrade, .Network, and the
-- item catalog. Returns (root, err).
...
-- Build a className/name -> itemID catalog from the root.
-- Always seeds the two ids proven from the deob (Wave=156, Ocean=50).
function M.catalog(root)
  local cat = {}
  local items = root and (root.Items or root.items)
  if type(items) == "table" then
    for k, v in pairs(items) do
      if type(v) == "table" then
        local id = v.id or v.Id or v.itemID or v.ItemID
        if v.class then cat[v.class] = cat[v.class] or id end
        if v.name then cat[v.name] = cat[v.name] or id end
        if type(k) == "string" then cat[k] = cat[k] or id end
...
-- Player's own gem balance: LocalPlayer.leaderstats.Gems.Value (deob-confirmed).
function M.gems()
...
-- Snapshot of the current trade from root.activeTrade (re-read fresh each call).
function M.activeTrade(root)
  if type(root) ~= "table" then return nil end
  return root.activeTrade
end

-- Current incoming invites table (map of uid -> invite entry).
function M.incomingInvites(root)
  if type(root) ~= "table" then return nil end
  return root.incomingInvites
end
...
function M.run(log, t)
  ...
  local function dumpRemotes(parent, label)
    ...
    local remotes = things:FindFirstChild("__REMOTES")
    if not remotes then log:warn("__REMOTES missing under " .. label .. ".__THINGS", t); return false end
    local names = {}
    for _, c in ipairs(remotes:GetChildren()) do names[#names + 1] = c.Name .. "(" .. c.ClassName .. ")" end
    log:ok(("__REMOTES under %s [%d]: %s"):format(label, #names, table.concat(names, ", ")), t)
    return true
  end
  if not dumpRemotes(workspace, "workspace") and not dumpRemotes(lp, "LocalPlayer") then
    log:warn("__THINGS not under workspace or LocalPlayer; searching descendants...", t)
    pcall(function()
      for _, d in ipairs(game:GetDescendants()) do
  ...
  pcall(function()
    ...
    if fw then
      ...
      -- base's real path: require(Framework:WaitForChild("Library"))
      local ok, root = pcall(function() return require(fw:WaitForChild("Library", 10)) end)
      if ok and type(root) == "table" then
        ...
        log:warn("Framework.Library require failed: " .. tostring(root), t)
      end
    else
...
-- deps: {
--   remotes,               -- resolved trade remotes
--   cfg, feed, catalog,    -- config, normalized feed, className->itemID map
--   getActiveTrade,        -- fn() -> root.activeTrade | nil
--   pickSellItem,          -- fn(activeTrade) -> setName | nil  (which item to sell)
--   sendTradeMsg,          -- fn(text)  -> trading_sendmessage:FireServer(text)
--   log, mode, gate, onSold, onBought,
-- }
function M.new(deps) return setmetatable({ d = deps, busy = false }, { __index = M }) end

-- Called per incoming invite entry (which carries .uid).
...
function M:accept(invite)
  -- Base: <acceptRemote>:FireServer(invite.uid)
...
function M:waitForTrade(timeout)
  ...
  repeat
    ...
    if at and at.offeredItemsTarget ~= nil then return at end
    task.wait(0.4)
...
function M:runSell(invite)
  ...
  local partner = at.targetName or (type(invite) == "table" and invite.name)
  ...
  if not setName then return self:abort("no sellable item selected") end
  ...
  for round = 0, (d.cfg.negotiation.maxRounds or 3) do
    ...
    if decision.action == "ACCEPT" then
      ...
      if d.onSold then d.onSold(setName, qty, fresh.offeredGemsTarget, cost) end
      return
...
function M:runBuy(invite)
  ...
  for round = 0, (d.cfg.negotiation.maxRounds or 3) do
    ...
    local decision = negotiation.decideBuy(d.cfg, bq, {
      sellerAsk = cur.offeredGemsTarget, round = round, elapsed = os.time() - start,
    })
    if decision.action == "ACCEPT" then
      local fresh = d.getActiveTrade() or cur
      ...
      d.remotes.trading_confirmtrade:FireServer()
      if d.onBought then d.onBought(setName, qty, fresh.offeredGemsTarget) end
      return
    elseif decision.action == "COUNTER" then
      if d.sendTradeMsg then d.sendTradeMsg(decision.message) end
      task.wait(d.cfg.negotiation.chatCooldownSec or 4)
...
-- ---- 4) debug report + UI early ----------------------------------------------
local function copyReport()
  local r = dbg:report("AutoTrade " .. CONFIG.version)
  local copied = false
  if setclipboard then copied = pcall(setclipboard, r) elseif toclipboard then copied = pcall(toclipboard, r) end
  print(r); return copied
end
...
local remotes, rroot
if wrongPlace then
  logwarn(("WRONG GAME: place=%s. Auto-trade only works in Trading Plaza (%s)."):format(tostring(game.PlaceId), tostring(cfg.expectedPlaceId)))
  ...
  logstep("disabling base SmartTrade file"); logok("disableBaseSmartTrade=" .. tostring(remotesM.disableBaseSmartTradeFile()))
  logstep("loading base plaza.lua")
  if remotesM.loadBase(CONFIG.baseUrl) then logok("base loaded") else logwarn("base FAILED to load") end
...
if remotes and rroot then
  ...
  local function getActiveTrade() return gamedata.activeTrade(rroot) end
  ...
  local function pickSellItem(at)
    local wants = at and (at.wantedItemsTarget or at.requestedItems)
    if type(wants) == "table" then
      for _, it in pairs(wants) do local c = (type(it) == "table" and (it.class or it.className)) or it; if c then return c end end
    end
    local best, bestVal
    for name, ov in pairs(cfg.items or {}) do
      if ov.forSale then local v = (feed[name] and feed[name].value) or 0
        if not bestVal or v > bestVal then best = name; bestVal = v end end
    end
    return best
  end
  local function pickBuyItem(at)
    ...
    if type(offered) == "table" then
      for _, it in pairs(offered) do
        local c = (type(it) == "table" and (it.class or it.className)) or it
        if c and cfg.buy[c] and cfg.buy[c].wantBuy then return c end
      end
    end
    return nil
  end
  ...
  -- Poll incomingInvites (the base's real signal) and dispatch new ones.
  local seen = {}
  survival.watchdog(cfg, "invite-loop", function()
    while true do
      if cfg.enabled then
        local inv = gamedata.incomingInvites(rroot)
        if type(inv) == "table" then
          ...
          for uid, entry in pairs(inv) do
            ...
            if not seen[uid] then
              seen[uid] = true
              local invite = type(entry) == "table" and entry or { uid = uid }
              invite.uid = invite.uid or uid
              logp("new invite uid=" .. tostring(uid))
              runner:handle(invite)
            end
          end
          for uid in pairs(seen) do if not present[uid] then seen[uid] = nil end end
        end
      end
      task.wait(1)
    end
...
elseif not wrongPlace then
  setStatus("Not fully wired (remotes=" .. tostring(remotes ~= nil) .. " root=" .. tostring(rroot ~= nil) .. "). Copy Debug Report.")
end
  ...
  task.spawn(function()
    while true do
      ...
      if fresh then for k in pairs(feed) do feed[k] = nil end
        ...
        logp("feed refreshed (" .. libFeed.count(feed) .. " items)") end
    end
  end)
end

logstep("ready. items=" .. libFeed.count(feed) .. " remotes=" .. tostring(remotes ~= nil) .. " root=" .. tostring(rroot ~= nil))
logp("If anything is off, press Diagnostics -> Copy Debug Report and send it back.")
