-- Bundled U232niversal Auto-Trade. Generated by tools/bundle.py. Do not edit.
...
function M.run(log, t)
  ...
  safe(log, "framework", t, function()
    ...
    logDump(log, "Library root keys", root, t, { maxDepth = 1, maxKeys = 60 })

    -- activeTrade (what the runner reads)
    if type(root.activeTrade) == "table" then
      logDump(log, "activeTrade", root.activeTrade, t, { maxDepth = 2 })
    else
      log:info("activeTrade = " .. tostring(root.activeTrade), t)
    end

    -- incomingInvites (the invite signal)
    if type(root.incomingInvites) == "table" then
      local n = 0
      for uid, entry in pairs(root.incomingInvites) do
        n = n + 1
        if n <= 2 then logDump(log, "incomingInvites[" .. tostring(uid) .. "]", (type(entry) == "table" and entry) or { value = entry }, t, { maxDepth = 2 }) end
      end
      log:ok("incomingInvites count = " .. n, t)
    else
      log:info("incomingInvites = " .. tostring(root.incomingInvites), t)
    end

    -- ITEM CATALOG (id per item — the all-item unlock)
    local items = root.Items or root.items
    if type(items) == "table" then
      local n = 0
      for k, v in pairs(items) do
        n = n + 1
        if n <= 5 then logDump(log, "Items[" .. tostring(k) .. "]", (type(v) == "table" and v) or { value = v }, t, { maxDepth = 1 }) end
      end
      log:ok("Items catalog count = " .. n, t)
    else
      log:warn("no root.Items catalog (check the root keys dumped above for the real name)", t)
    end
  end)
...
if remotes and rroot then
  ...
  survival.watchdog(cfg, "invite-loop", function()
    while true do
      ...
      local invCount = 0
      if type(inv) == "table" then for _ in pairs(inv) do invCount = invCount + 1 end end
      -- heartbeat every ~15s so the debug report shows live state
      if ticks % 15 == 1 then
        local at = getActiveTrade()
        logp(("heartbeat: enabled=%s invites=%s activeTrade=%s gems=%s")
          :format(tostring(cfg.enabled), tostring(invCount), tostring(at ~= nil), tostring(getGems())))
      end
      if cfg.enabled and type(inv) == "table" then
        local present = {}
        for uid, entry in pairs(inv) do
          ...
          if not seen[uid] then
            ...
            logp("NEW INVITE uid=" .. tostring(uid) .. " -> handling")
