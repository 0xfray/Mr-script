...
-- Source of truth: src/*11111.lua  (regenerate: python tools/bundle.py)
local __M = {}
...
-- Category tables on the root that hold tradeable item definitions.
local ITEM_CATEGORIES = { "Appliance", "Furniture", "Food", "Cook", "Bakery", "Floor", "Directory" }

-- Extract a name -> itemID map by scanning the category tables.
-- Handles several plausible entry shapes; seeds the deob-proven ids.
  ...
  local function absorb(tbl)
    ...
    for k, v in pairs(tbl) do
      if type(v) == "table" then
        local id = v.id or v.Id or v.itemID or v.ItemID or v.ID
        local name = v.name or v.Name or v.class or v.className
        if name and id then cat[name] = cat[name] or id end
        if type(k) == "string" and id then cat[k] = cat[k] or id end
        if type(k) == "number" and name then cat[name] = cat[name] or k end
      elseif type(k) == "string" and (type(v) == "number" or type(v) == "string") then
        cat[k] = cat[k] or v
      end
    end
  end
  for _, catego in ipairs(ITEM_CATEGORIES) do absorb(root and root[catego]) end
...
-- Current trade status via the trading_getstatus RemoteFunction.
-- Returns the status table (activeTrade-equivalent) or nil.
...
if wrongPlace then
  ...
  local rerr; remotes, rerr = remotesM.resolveRemotes(30)
  ...
  local rooterr; rroot, rooterr = gamedata.root(30)
...
if remotes and rroot then
  ...
  -- one-time shape dump of the status object
  local dumpedStatus = false
  local function getActiveTrade()
    local st = gamedata.getStatus(remotes)
    if st and not dumpedStatus then dumpedStatus = true; logDump("trading_getstatus result", st) end
    return st
  end
  ...
  local function pickSellItem(at)
    local wants = at and (at.wantedItemsTarget or at.requestedItems or at.offeredItemsPlayer)
    if type(wants) == "table" then for _, it in pairs(wants) do local c = (type(it) == "table" and (it.class or it.className or it.name)) or it; if c then return c end end end
    local best, bestVal
    for name, ov in pairs(cfg.items or {}) do if ov.forSale then local v = (feed[name] and feed[name].value) or 0; if not bestVal or v > bestVal then best = name; bestVal = v end end end
    return best
  end
  ...
  -- Invite detection via the trading_invitesent RemoteEvent (report-confirmed).
  local dumpedInvite = false
  local function onInvite(...)
    local args = { ... }
    if not dumpedInvite then dumpedInvite = true; logDump("trading_invitesent args", args) end
    if not cfg.enabled then return end
    local a1 = args[1]
    local uid = (type(a1) == "table" and (a1.uid or a1.Uid or a1.id or a1.userId or a1.UserId))
             or (type(a1) == "number" and a1)
             or a1
    local invite = (type(a1) == "table" and a1) or { uid = uid }
    invite.uid = invite.uid or uid
    logp("INVITE received uid=" .. tostring(invite.uid))
    runner:handle(invite)
  end
  if remotes.trading_invitesent then
    remotes.trading_invitesent.OnClientEvent:Connect(onInvite)
    logok("listening on trading_invitesent")
  else
    logwarn("trading_invitesent remote absent - cannot detect invites")
  end

  -- Log shapes of the other trade events (one-time) for confirmation.
  local dumpedEvt = {}
  local function watchEvent(name)
    local r = remotes[name]
    if r and r.OnClientEvent then
      r.OnClientEvent:Connect(function(...)
        if not dumpedEvt[name] then dumpedEvt[name] = true; logDump(name .. " args", { ... }) end
      end)
    end
  end
  watchEvent("trading_tradebegan"); watchEvent("trading_statuschange"); watchEvent("trading_updategemoffer")

  -- Heartbeat so the report shows live state.
  local ticks = 0
  survival.watchdog(cfg, "heartbeat", function()
    while true do
      ticks = ticks + 1
      if ticks % 15 == 1 then
        local at = getActiveTrade()
        logp(("heartbeat: enabled=%s activeTrade=%s gems=%s"):format(tostring(cfg.enabled), tostring(at ~= nil), tostring(getGems())))
      end
      task.wait(1)
    end
  end, onEvent)
